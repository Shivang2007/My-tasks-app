...
class Categories(Screen):
    ...
    def toadytasktemp(self):
        try:
            with open('texts/main_path.txt','r') as f:
                MP = f.read()
            lst = os.listdir(f'{MP}')
            if 'Today' in lst:
                toast('Folder named Today is already there')
            else:
                ct = time.localtime()
                dt = f'{ct[2]}/{ct[1]}/{ct[0]}'
                tt = f'{ct[3]}:{ct[4]}:{ct[5]}'
                ...
                with open('path.txt','w') as f:
                    f.write(f'Today')
                with open(f'{MP}/Today/desc.color','w') as f:
                    f.write('Things i have to do today')
                with open(f'{MP}/Today/time.color','w') as f:
                    f.write(str(dt))
                with open(f"{MP}/Today/super_label.color",'w') as f:
                    f.write(f"Today's Tasks&&&&Things i have to do today")       
                with open(f"{MP}/Today/color.color",'w') as f:
                    f.write('yellow')
                self.enter()                    
                Snackbar(text='Folder Made Succussfully',bg_color='blue').open()
        except Exception as e:
            toast(f"{e}")
